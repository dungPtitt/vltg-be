...
exports.register = async (req, res, next) => {
  try {
    ...
    if (userName && email && password) {
      ...
      if (checkEmail)
        return functions.setError(res, "Email already exists", 400);
...
exports.thongKeDanhSachUngVien = async (req, res, next) => {
  try {
    ...
    let id_ntd = req.user && req.user.data ? req.user.data._id : null;
    for (let i = 0; i < danhSachUngVien.length; i++) {
      danhSachUngVien[i].linkAvatar = vltgService.getLinkFile(
        "user_uv",
  ...
  try {
    ...
    if (id_uv) {
      ...
      if (ungVien) {
        ...
        ungVien.linkAvatar = vltgService.getLinkFile(
          "user_uv",
          ungVien.createdAt,
          ungVien.avatarUser
        );
...
exports.getInfoCompany = async (req, res, next) => {
  try {
    ...
    if (ntd) {
      ...
      ntd.linkAvatar = vltgService.getLinkFile(
        folder_img,
        time_created,
        ntd.avatarUser
      );
    ...
    if (ntd) {
      if (req.files && req.files.avatar) {
        ...
        if (check) {
          ...
          let nameAvatar = await vltgService.uploadFileNameRandom(
            folder_img,
            time_created,
            avatar
          );
