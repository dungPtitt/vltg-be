...
exports.uploadFileDocTmpCV = multer({ storage: storageDocCVTmp('../storage/base365/timviec365/cv365/tmp') });
...
exports.getImageUv = (createTime, logo) => {
    if (logo != '' && logo != null) {
        return `${process.env.cdn}/pictures/uv/${this.convertDate(createTime, true)}/${logo}`;
    }
    return "";
}
...
exports.getMaxIdByFieldWithDeleted = async(model, field) => {
    let maxId = await model.findOneWithDeleted({}, {
        [field]: 1
    }).sort({
        [field]: -1
    }).limit(1).lean();
